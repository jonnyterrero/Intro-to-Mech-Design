...
// ================================================================
//  TYPE DEFINITIONS — must come before any function that uses them
// ================================================================
...
// ----------------------------------------------------------------
//  Calibration (corrected for servo direction)
// ----------------------------------------------------------------
const int HAND_OPEN   = 0;
const int HAND_CLOSED = 90;

const int WRIST_UP    = 150;   // physically raised for travel
const int WRIST_DOWN  = 60;    // physically at table for pickup
const int WRIST_DROP  = 90;    // halfway — releasing over cup

const int FOREARM_POS  = 45;   // retracted — safe travel clearance
const int FOREARM_GRAB = 105;  // extended — reaches ground
...
// ----------------------------------------------------------------
//  Sensor filter — median-of-3 + EWMA
// ----------------------------------------------------------------
float pingBuf[3];
uint8_t pingIdx    = 0;
bool    pingFull   = false;
...
void loop() {
  ...
  switch (state) {
    ...
    case ST_APPROACH: {
      jointMoveTo(jHand, HAND_OPEN);
      // REMOVED: jointMoveTo(jWrist, WRIST_UP) — caused unnecessary raise
    ...
    case ST_GRAB: {
     ...
     case ST_LIFT: {
      jointMoveTo(jHand, HAND_CLOSED);      // ADDED: reinforce grip
      jointMoveTo(jWrist, WRIST_UP);
      jointMoveTo(jForearm, FOREARM_POS);
    ...
    case ST_RELEASE: {
      ...
      // phase 1: wait for ball to clear, then go straight to RETURN
      // REMOVED old phase 2 (wrist raise) — caused unnecessary lift before return
      else if (phase == 1) {
